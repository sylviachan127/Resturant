...
public class CharacterMotor : MonoBehaviour
{
    ...
    float MaxSpeedInDirection(Vector3 desiredMovementDirection)
    {
        ...
        {
			float speedModifier = Input.GetKey(KeyCode.LeftShift) ? 2 : 1;
            float zAxisEllipseMultiplier = (desiredMovementDirection.z > 0 ? movement.maxForwardSpeed : movement.maxBackwardsSpeed) / movement.maxSidewaysSpeed * speedModifier;
            Vector3 temp = new Vector3(desiredMovementDirection.x, 0, desiredMovementDirection.z / zAxisEllipseMultiplier).normalized;
            float length = new Vector3(temp.x, 0, temp.z * zAxisEllipseMultiplier).magnitude * movement.maxSidewaysSpeed * speedModifier;
            return length;
        }
    }
